...
export const runLeadAutoCategorizeThunk = (): AppThunk<ResponseType> => {
  return async (dispatch, getState): ResponseType => {
    ...
    const result = await postLeadAutoCategorizeForLead(leadId);
    const json = (result.data ?? result) as PostLeadAutoCategorizeResponseBody;

    if (!result.success || json.success === false) {
      return {
        ok: false,
        status: !result.success ? mapApiFailureToThunkStatus(result) : 500,
        error: typeof json.error === 'string' ? json.error : result.error,
        message: typeof json.error === 'string' ? json.error : result.error,
      };
    }
