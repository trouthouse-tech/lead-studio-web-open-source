...
export const LeadContactEmailQueueList = () => {
  ...
  const [sortColumn, setSortColumn] = useState<
    'status' | 'contact' | 'lead' | 'scheduled_at' | 'sent_at' | 'notes' | 'error'
  >('scheduled_at');
  const [sortDirection, setSortDirection] = useState<'asc' | 'desc'>('desc');
  const toCallLogByQueueId = useMemo(() => {
    const nextByQueueId: Record<string, ToCallLog> = {};
    Object.values(toCallLogs).forEach((item) => {
      if (!item.lead_contact_email_queue_id) return;
      if (!nextByQueueId[item.lead_contact_email_queue_id]) {
        nextByQueueId[item.lead_contact_email_queue_id] = item;
      }
    });
    return nextByQueueId;
  }, [toCallLogs]);
  ...
  const handleRefresh = async () => {
    ...
    await dispatch(getAllToCallLogThunk({ force: true }));
    ...
    filtered = [...filtered].sort((a, b) => {
      ...
      switch (sortColumn) {
        ...
        case 'sent_at':
          ...
          break;
        case 'notes':
          aValue = (toCallLogByQueueId[a.id]?.notes || '').toLowerCase();
          bValue = (toCallLogByQueueId[b.id]?.notes || '').toLowerCase();
          break;
        case 'error':
  ...
  const handleViewContact = (leadId: string) => {
    dispatch(setCurrentLeadThunk(leadId));
    router.push(LEAD_DETAIL_PATH);
  };
  ...
  const handleRequeue = async (item: LeadContactEmailQueue) => {
    setActionLoadingItemId(item.id);
    const result = await dispatch(requeueQueueItemThunk(item));
    if (result !== 200) {
      alert('Failed to re-queue item');
      setActionLoadingItemId(null);
      return;
    }
    await dispatch(getAllLeadContactEmailQueueThunk());
              ...
              {filteredItems.map((item) => {
                ...
                return (
                  <tr key={item.id} className={styles.tableRow}>
                    ...
                    <td className={styles.tableCell}>
                      {contact ? (
                        <span
                          ...
                          onClick={() => handleViewContact(item.lead_id)}
                    ...
                    <td className={styles.tableCell}>
                      {toCallLogByQueueId[item.id]?.notes ? (
                        <span
                          className={styles.callNotesText}
                          title={toCallLogByQueueId[item.id].notes}
                        >
                          {toCallLogByQueueId[item.id].notes.length > 60
                            ? `${toCallLogByQueueId[item.id].notes.substring(0, 60)}...`
                            : toCallLogByQueueId[item.id].notes}
                        </span>
                      ) : (
                        'N/A'
...
const styles = {
  ...
  `,
  actionsButton: `
    px-2 py-1 text-sm hover:bg-gray-100 rounded transition-colors cursor-pointer
    text-gray-600 border border-gray-200 bg-white min-w-[28px]
  `,
  actionsMenu: `
    absolute right-0 top-8 z-20 min-w-[120px]
  ...
  actionMenuItemDelete: `
    w-full text-left px-3 py-1.5 text-xs text-red-600
