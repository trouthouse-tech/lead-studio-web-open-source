...
import {
  deleteLeadSentEmailThunk,
  openSentEmailContactPanelThunk,
} from '@/store/thunks/lead-sent-emails';
import type {
  LeadSentEmail,
...
export const LeadSentEmailRow = (props: LeadSentEmailRowProps) => {
  ...
  const handleOpenContactPanel = () => {
    if (!contact) return;
    void dispatch(openSentEmailContactPanelThunk(contact.id));
  };
      ...
      <td className={styles.tableCell}>
        {contact ? (
          <div className={styles.contactCell}>
            <button
              type="button"
              onClick={handleOpenContactPanel}
              className={styles.clickableName}
              title={contact.email ?? 'Open contact follow-up panel'}
            >
...
const styles = {
  ...
  `,
  contactCell: `flex flex-col items-start gap-1 sm:flex-row sm:items-center sm:gap-2`,
  clickableName: `
    text-left text-blue-600 hover:text-blue-800 hover:underline transition-colors text-xs cursor-pointer
    border-none bg-transparent p-0
  `,
  leadLink: `
    text-[10px] font-medium text-gray-500 hover:text-blue-700 hover:underline
    border-none bg-transparent p-0 cursor-pointer shrink-0
  `,
  loadingText: `
